...
class MainScreenOld extends StatefulWidget {
  ...
  @override
  State<MainScreenOld> createState() => _MainScreenOld();
}

class _MainScreenOld extends State<MainScreenOld> with TickerProviderStateMixin {
  var _user = User();
  var _planningData = PlanningData();
  var _votingStory = Story();

  List<StoryVote> _storyVotes = [];
                  ...
                  return Column(
                    ...
                    children: [
                      StoryCard.newCard(
                          ...
                          //size: Size(Consts.storyCardWidth - 20, 140),
                          context: context,
                  ...
                  children: [
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          var storyCard = StoryCard(
                            ...
                            size: Size(Consts.storyCardWidth, 140),
                          );
                          if (_user.creator) {
                            return index == 0
                                ? Row(
                    ...
                    _votingStory.id.isEmpty
                        ...
                        : StreamBuilder<QuerySnapshot>(
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                                _storyVotes = snapshot.data!.docs.map((e) => StoryVote.fromDocument(e)).toList();
                              } else {
                                _storyVotes = [];
                              }

                              var storyVote = _storyVotes.where((vote) => vote.userId == _user.id).firstOrNull ?? StoryVote();

                              var votesAvarege = 0.0;
                              var minVote = 0;
                              var maxVote = 0;

                              if (_storyVotes.isNotEmpty) {
                                var votesPoints = _storyVotes.map((e) => e.points).toList();
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child: Column(
                                      children: [
                                        // vote data
                                        if (((_user.creator || _user.isSpectator) && _storyVotes.isNotEmpty) ||
                                        ...
                                        if ((_user.creator || _user.isSpectator) ||
                                            (_user.isPlayer && _votingStory.status == StoryStatus.votingFinished))
                                          VoteCard.castedVotesList(context: context, user: _user, storyVotes: _storyVotes),
                                  ...
                                  Container(
                                    ...
                                    child: StreamBuilder<QuerySnapshot>(
                                      stream: UserController().getUserListData(
                                        planningId: _planningData.id,
                                      ),
                                      builder: (context, snapshot) {
                                        List<User> users = [];
                                            ...
                                            ListView.builder(
                                              ...
                                              itemBuilder: (context, index) {
                                                return Card(
                                                  child: ListTile(
                                                    dense: true,
                                                    title: TebText(players[index].name, textSize: 15),
                                                    trailing: _storyVotes.where((v) => v.userId == players[index].id).isNotEmpty
                                            ...
                                            ListView.builder(
                                              ...
                                              itemBuilder: (context, index) {
                                                return Card(
                                                  child: ListTile(
                                                    dense: true,
                                                    title: TebText(spectators[index].name, textSize: 15),
                                                  ),
                                                );
                                              },
                                            )
